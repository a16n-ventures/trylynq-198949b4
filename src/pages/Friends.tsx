...
const Friends = () => {
  ...
  const { data: suggestions = [] } = useQuery({
    queryKey: ['suggestions', location?.latitude, user?.id, acceptedFriendIds],
    queryFn: async () => {
  ...
  const friendshipMutation = useMutation({
    mutationFn: async ({ action, id, targetId }: { action: 'connect' | 'accept' | 'decline' | 'unfriend' | 'block', id?: string, targetId?: string }) => {
      switch (action) {
    ...
    onSuccess: (_, variables) => {
      toast.success(`Action successful: ${variables.action}`);
  ...
  }, [friends, allContacts, acceptedFriendIds, searchQuery, user?.id]);
          ...
          <TabsContent value="circle" className="space-y-6">
            ...
            {suggestions.length > 0 && (
              <div className="flex gap-3 overflow-x-auto pb-4 scrollbar-hide">
                {suggestions.map((s: any) => (
                  <div key={s.user_id} className="min-w-[140px] p-4 bg-card border rounded-2xl text-center">
                    <Avatar className="mx-auto h-16 w-16 mb-2"><AvatarImage src={s.avatar_url} /><AvatarFallback>{s.display_name[0]}</AvatarFallback></Avatar>
                    <p className="text-sm font-bold truncate">{s.display_name}</p>
                    <Button size="sm" className="w-full mt-2 h-7 text-xs" onClick={() => friendshipMutation.mutate({ action: 'connect', targetId: s.user_id })}>Connect</Button>
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="space-y-2">
              {filteredFullList.map(friend => (
                <div key={friend.user_id} className="flex items-center gap-3 p-3 bg-card border rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold text-sm">{friend.display_name}</h4>
                    <p className="text-xs text-muted-foreground truncate">@{friend.username}</p>
                  </div>
                  {friend.friendship_id === 'contact' ? (
                    <Button size="sm" onClick={() => friendshipMutation.mutate({ action: 'connect', targetId: friend.user_id })}>Add</Button>
                  ) : (
          ...
          <TabsContent value="requests" className="space-y-3">
            {requests.map((req: any) => (
              <div key={req.id} className="flex items-center gap-3 p-4 bg-card border rounded-xl">
                ...
                <div className="flex-1"><h4 className="text-sm font-bold">{req.requester.display_name}</h4></div>
                <div className="flex gap-2">
                  <Button size="icon" variant="outline" className="rounded-full" onClick={() => friendshipMutation.mutate({ action: 'decline', id: req.id })}><X className="w-4 h-4" /></Button>
                  <Button size="icon" className="rounded-full bg-green-600" onClick={() => friendshipMutation.mutate({ action: 'accept', id: req.id })}><Check className="w-4 h-4" /></Button>
                </div>
              </div>
            ))}
          </TabsContent>
