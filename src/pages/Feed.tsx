                ...
                <TabsContent value={activeTab} className="mt-0 space-y-5 px-4 min-h-[50vh]">
                  {activeTab === 'communities' ? (
                    <div className="space-y-3">
                      ...
                      communities.map(c => (
                        <div key={c.id} className="relative w-full h-48 items-center gap-4 bg-card rounded-xl border shadow-sm cursor-pointer hover:bg-accent/50" onClick={() => setSelectedCommunity(c)}>
                          <Avatar className="absolute inset-0 w-full h-full"><AvatarImage src={c.avatar_url || undefined} className="object-cover" /><AvatarFallback>{c.name[0]}</AvatarFallback></Avatar>
