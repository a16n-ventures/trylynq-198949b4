...
import { Check, Wrench, Loader2 } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { toast } from "sonner";
import { useAuth } from "@/contexts/AuthContext";

// ─────────────────────────────────────────────────────────────────────────────
// Trade / skill taxonomy
// Add or reorder freely — these drive the service-provider onboarding.
// ─────────────────────────────────────────────────────────────────────────────
...
export function SkillSelector({
  ...
  const saveSkills = async () => {
    ...
    try {
      ...
      setLoading(false); // Only reset on error; let loader show during navigation
    }
  };
    ...
    <div className="flex flex-col h-full max-w-md mx-auto p-6 animate-in fade-in slide-in-from-bottom-4">
      <div className="text-center mb-6 space-y-2">
        ...
        <p className="text-muted-foreground text-sm">
          Pick every skill or trade that applies. This is how people discover
          and hire you.
        </p>
      </div>
