CREATE OR REPLACE FUNCTION public.generate_smart_feed(p_user_id uuid, p_user_lat double precision DEFAULT NULL::double precision, p_user_long double precision DEFAULT NULL::double precision, p_city text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Communities — alias live count to live_member_count to avoid ambiguity
  WITH comm_pool AS (
    SELECT
      c.id,
      c.name,
      ...
      c.join_fee,
      c.created_at,
      (SELECT count(*) FROM community_members cm WHERE cm.community_id = c.id) as live_member_count,
  ...
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      ...
      'member_count', sc.live_member_count,
